...
const renderContent = (content, onMentionClick, onImageClick) => {
  ...
  return parts.map((part, i) => {
    ...
    if (part.match(mentionRegex)) return <span key={i} className="mention-highlight" style={{ cursor: 'pointer' }} onClick={() => onMentionClick(part.slice(1))}>{part}</span>;
    return part;
  });
};
...
const groupMessages = (messages) => {
  ...
  messages.forEach((msg) => {
    if (msg.system) { groups.push({ type: 'system', content: msg.content, id: msg.id }); return; }
    ...
    if (last?.type === 'group' && last.user_id === msg.user_id && !msg.reply_to_id) {
      ...
      groups.push({ type: 'group', user_id: msg.user_id, username: msg.username, avatar_color: msg.avatar_color, avatar_url: msg.avatar_url, messages: [msg] });
    }
  });
  return groups;
};
...
function ReactionPicker({ onPick, position }) {
  const emojis = ['👍','👎','❤️','😂','😮','😢','🔥','💯','🎉','👏'];
  return (
    <div className="reaction-picker" style={{ position: 'fixed', top: position.top, left: position.left, zIndex: 9999 }} onMouseDown={e => e.stopPropagation()}>
      {emojis.map((emoji, i) => (
        <button key={i} className="reaction-picker-btn" onMouseDown={e => { e.preventDefault(); e.stopPropagation(); onPick(emoji); }}>{emoji}</button>
      ))}
    </div>
  );
}
...
export default function ChatArea({ channel }) {
  ...
  useEffect(() => {
    const handler = (e) => {
      if (!e.target.closest('.reaction-picker') && !e.target.closest('.msg-react-btn')) setActiveReactionPicker(null);
    };
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      socket.emit('send_message', { channelId: channel.id, content: `[img]${url}[/img]`, replyToId: replyTo?.id || null });
      setReplyTo(null);
    } catch (err) { console.error(err); }
  };
  ...
  useEffect(() => {
    if (!channel) return;
    setMessages([]); setReactions({}); setLoading(true); setReplyTo(null);
    api.get(`/api/channels/${channel.id}/messages`).then(({ data }) => { setMessages(data); setLoading(false); });
    ...
    const onTypingStart = ({ username }) => { if (username !== user.username) setTyping(prev => [...new Set([...prev, username])]); };
    const onTypingStop = ({ username }) => setTyping(prev => prev.filter(u => u !== username));
    const onAvatarUpdated = ({ userId, avatar_url }) => setMessages(prev => prev.map(m => String(m.user_id) === String(userId) ? { ...m, avatar_url } : m));
    const onUsernameUpdated = ({ userId, username }) => setMessages(prev => prev.map(m => String(m.user_id) === String(userId) ? { ...m, username } : m));
    const onMessageEdited = ({ messageId, content }) => setMessages(prev => prev.map(m => m.id === messageId ? { ...m, content, edited: true } : m));
    const onMemberJoined = ({ member, serverId }) => {
      if (channel?.server_id === serverId) {
        setMessages(prev => [...prev, { id: `join-${member.id}-${Date.now()}`, content: `${member.username} has joined the server!`, system: true, created_at: new Date().toISOString() }]);
      }
    };
  ...
  const sendMessage = (e) => {
    ...
    socket.emit('send_message', { channelId: channel.id, content: input.trim(), replyToId: replyTo?.id || null });
    setInput('');
    setShowEmoji(false);
    setReplyTo(null);
  ...
  const handleKeyDown = (e) => {
    if (e.key === 'Enter' && !e.shiftKey) { sendMessage(e); return; }
    if (e.key === 'Escape' && replyTo) { setReplyTo(null); return; }
    socket.emit('typing_start', { channelId: channel.id });
    clearTimeout(typingTimeout.current);
    typingTimeout.current = setTimeout(() => socket.emit('typing_stop', { channelId: channel.id }), 2000);
  };
  ...
  return (
    <div className="chat-area">
      ...
      <div className="chat-messages">
        ...
        {groups.map((group, i) => {
          if (group.type === 'system') return <div key={`system-${i}`} className="system-message">🎉 {group.content}</div>;
        ...
        {typing.length > 0 && (
          <div className="typing-indicator fade-in">
            ...
            <span className="typing-text">{typing.join(', ')} {typing.length === 1 ? 'is' : 'are'} typing...</span>
          </div>
        )}
        <div ref={bottomRef} />
      </div>
      ...
      {activeReactionPicker && createPortal(
        <ReactionPicker onPick={(emoji) => toggleReaction(activeReactionPicker, emoji)} position={pickerPosition} />,
        document.body
      )}

      {showMentions && (
        <div className="mention-picker">
          {members.filter(m => m.username.toLowerCase().startsWith(mentionSearch.toLowerCase())).slice(0, 5).map(m => (
            <button key={m.id} className="mention-item" onClick={() => {
              const atIndex = input.lastIndexOf('@');
              setInput(input.slice(0, atIndex) + `@${m.username} `);
              setShowMentions(false);
            }}>
              <div className="mention-avatar" style={{ background: m.avatar_color }}>{m.username[0].toUpperCase()}</div>
        ...
        <form className="chat-input-form" onSubmit={sendMessage}>
          <textarea
            ...
            onChange={e => {
              ...
              if (atIndex !== -1 && atIndex === val.length - 1) { setShowMentions(true); setMentionSearch(''); }
              else if (atIndex !== -1 && val.slice(atIndex).match(/^@\w*$/)) { setShowMentions(true); setMentionSearch(val.slice(atIndex + 1)); }
              else setShowMentions(false);
            }}
            onKeyDown={handleKeyDown}
            placeholder={replyTo ? `Replying to ${replyTo.username}...` : `Message #${channel.name}`}
            rows={1}
          />
