...
export default function TypeRacer({ onClose }) {
  ...
  const handleInput = (e) => {
    ...
    if (val === sentence) {
      ...
      setWpm(finalWpm);
      setGameState('done');
      api.post('/api/scores', { game: 'type', score: finalWpm, meta: { accuracy: acc, errors: errs } }).then(({ data }) => { if (data.ticketsEarned > 0) setTicketsEarned(data.ticketsEarned); }).catch(() => {});
    }
  };
