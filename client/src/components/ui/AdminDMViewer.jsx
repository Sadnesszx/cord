...
export default function AdminDMViewer({ onClose }) {
  ...
  useEffect(() => {
    if (tab === 'reports') {
      ...
      api.get('/api/admin/reports').then(({ data }) => {
        setReports(data);
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', borderBottom: '1px solid #2a2a2a' }}>
          <button onClick={() => setTab('dms')} style={{ flex: 1, padding: '10px', background: tab === 'dms' ? '#1a1a1a' : 'none', border: 'none', borderBottom: tab === 'dms' ? '2px solid #5865f2' : '2px solid transparent', color: tab === 'dms' ? '#e8e8e8' : '#888', fontSize: 13, cursor: 'pointer', fontWeight: tab === 'dms' ? 600 : 400 }}>
            💬 DM Viewer
          </button>
          <button onClick={() => setTab('reports')} style={{ flex: 1, padding: '10px', background: tab === 'reports' ? '#1a1a1a' : 'none', border: 'none', borderBottom: tab === 'reports' ? '2px solid #f23f43' : '2px solid transparent', color: tab === 'reports' ? '#e8e8e8' : '#888', fontSize: 13, cursor: 'pointer', fontWeight: tab === 'reports' ? 600 : 400 }}>
            ⚠️ Reports {reports.length > 0 && <span style={{ background: '#f23f43', color: '#fff', fontSize: 10, fontWeight: 700, padding: '1px 5px', borderRadius: 99, marginLeft: 4 }}>{reports.length}</span>}
          </button>
        </div>
              ...
              {messages.map(msg => (
                <div key={msg.id} style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                  {msg.avatar_url ? (
                    ...
                    <div style={{ width: 32, height: 32, borderRadius: '50%', background: msg.avatar_color, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, color: '#fff', flexShrink: 0 }}>{msg.username[0].toUpperCase()}</div>
                  )}
                  <div>
            ...
            {reports.map(r => (
              <div key={r.id} style={{ background: '#1a1a1a', border: '1px solid #2a2a2a', borderRadius: 10, padding: 14 }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <button onClick={() => resolveReport(r.id)} style={{ background: '#23a55a', color: '#fff', border: 'none', borderRadius: 6, padding: '4px 12px', fontSize: 12, cursor: 'pointer', fontWeight: 600 }}>
                    ✓ Resolve
                  </button>
                </div>
              </div>
