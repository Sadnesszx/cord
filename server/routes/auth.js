...
router.post('/register', async (req, res) => {
  ...
  try {
    ...
    const { rows } = await pool.query(
      `INSERT INTO users (username, email, password_hash, birthday) VALUES ($1, $2, $3, $4) RETURNING id, username, avatar_color`,
    ...
    try {
      ...
      if (io) io.emit('new_user', { username: user.username });
...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      await pool.query(
        `DELETE FROM user_sessions WHERE user_id = $1 AND id NOT IN (SELECT id FROM user_sessions WHERE user_id = $1 ORDER BY created_at DESC LIMIT 10)`,
        [user.id]
      );
